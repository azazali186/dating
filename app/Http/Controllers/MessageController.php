...
class MessageController extends Controller
{
    public function index()
    {
        ...
        if ($role == 'user') {
            $messages = Message::with(['user', 'seller'])->groupBy('seller_id')->orderBy('created_at', 'asc')->where('user_id', Session::get('sessdata')['id'])->get();
        }
        if ($role == 'seller') {
            $messages = Message::with(['user', 'seller'])->groupBy('user_id')->orderBy('created_at', 'asc')->where('seller_id', Session::get('sessdata')['id'])->get();
        }
